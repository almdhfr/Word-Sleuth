...
namespace UVNF.Core
{
    public class UVNFManager : MonoBehaviour
    {
        ...
        public delegate void StoryFinishedEventHandler(string storyName);
        public event StoryFinishedEventHandler StoryFinished;
        public delegate void StoryStartedEventHandler(string storyName);
        public event StoryFinishedEventHandler StoryStarted;
        ...
        public bool StartStory(StoryGraph graph)
        {
            if (_currentStoryManager == null)
            {
                ...
                _currentStoryName = ((StartElement)graph.GetRootStory()[0]).StoryName;

                // Debug.Log($"Starting Story: {_currentStoryName}");
                StoryStarted?.Invoke(_currentStoryName);
        ...
        {   
            var finishedStory = _currentStoryName;
            ...
            StoryFinished?.Invoke(finishedStory);
        }
    }
