...
namespace UVNF.Core
{
    ...
    {
        public List<Character> CharactersOnScreen = null;
        public RectTransform MainCharacterStack;

        private CanvasGroup allCharactersCanvasGroup;

        private void Awake()
        {
            allCharactersCanvasGroup = GetComponent<CanvasGroup>();
        }
        ...
        public void AddCharacter(string characterName, Sprite characterSprite, bool flip, ScenePositions enter, ScenePositions position, float enterTime)
        {
            ...
            switch (enter)
            {
                case ScenePositions.Left:
                    startPosition = new Vector2(-parentTransform.sizeDelta.x - spriteTransform.sizeDelta.x / 1, 0);
                    break;
                case ScenePositions.Top:
                    startPosition = new Vector2(0, parentTransform.sizeDelta.y + spriteTransform.sizeDelta.y / 1);
                    break;
            ...
            {
                case ScenePositions.Left:
                    endPosition = new Vector2(-(parentTransform.sizeDelta.x / 1.25f), 0);
        ...
        public void RemoveCharacter(string characterName, ScenePositions exitPosition, float exitTime)
        {
            ...
            {
                case ScenePositions.Left:
                    endPosition = new Vector3(-(character.Parent.rect.width + (character.Transform.rect.width / 1f)), 0, 0);
                    break;
                case ScenePositions.Top:
            ...
            StartCoroutine(FadeCharacter(character, true, exitTime));
        ...
        private IEnumerator FadeCharacter(Character character, bool fade, float fadeTime)
        {
            float start = fade ? 1 : 0;
            float end = fade ? 0 : 1;
            ...
            while (currentLerpTime < fadeTime)
            {
                ...
                if (currentLerpTime > fadeTime)
                    currentLerpTime = fadeTime;

                float t = currentLerpTime / fadeTime;
//                t = t * t * t * (t * (6f * t - 15f) + 10f);
                character.SpriteRenderer.color = new Color(1, 1, 1, Mathf.Lerp(start, end, t));
                yield return null;
            }
        }
    }
